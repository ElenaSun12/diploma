...
public class MainPage {
    ...
    private final int buttonMenuId = R.id.main_menu_image_button; // кнопка меню с (тремя полосками)
    private final int menuSectionId = R.id.trademark_image_view; // картинка с надписью В хосписе
    private final int buttonOurMissionId = R.id.our_mission_image_button;  // картинка с бабочкой

    //private final int NewsSectionId = R.id.news_list_swipe_refresh; // Надпись News
    private final int logoutButtonId = R.id.authorization_image_button; // кнопка с человечком "Выйти"

    // Ждем появления элементов главного экрана
    public void waitForMainScreen() {
        int[] mainElements = {buttonMenuId, menuSectionId, logoutButtonId, buttonOurMissionId};
        onView(isRoot())
                .perform(ViewHelper
                        .waitForAnyElement(mainElements, 10000));
    }

    // Метод для разлогинивания
    public void logout() {
        // Нажимаем на иконку с человечком
        onView(withId(logoutButtonId)).perform(click());

        // Ищем и нажимаем кнопку "Выйти/Logout"
        try {
            onView(withText("Выйти")).perform(click());
        } catch (Exception e) {
            onView(withText("не видно кнопки Выйти")).perform(click());
        }
    }

    public void checkbuttonMenuIsDisplayed() {
        onView(withId(buttonMenuId))
                .check(matches(isDisplayed()));
    }

    public boolean checkTrademarkIsDisplayed() {
        try {
            onView(withId(menuSectionId)).check(matches(isDisplayed()));
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public void checkLogoutButtonIsDisplayed() {
        onView(withId(logoutButtonId))
                .check(matches(isDisplayed()));
    }

//    public void checkTrademarkIsDisplayed() {
//        onView(withId(logoutButtonId))
//                .check(matches(isDisplayed()));
//    }

    public void checkMenuSectionIsDisplayed() {
        // Проверяем значок бабочки как заголовок
        onView(withId(menuSectionId))
                .check(matches(isDisplayed()));
    }

    //проверяем наличие кнопки Выйти и кнопки меню Три полоски
    public void checkMainElementsAreDisplayed() {
        onView(withId(logoutButtonId))
                .check(matches(isDisplayed()));
        onView(withId(menuSectionId))
                .check(matches(isDisplayed()));
...
//    public void checkAllMainElements() {
//        checkUserMenuIsDisplayed();
//        checkMainSectionsAreDisplayed();
//        checkMainTitleIsDisplayed();
//    }

//    public void logout() {
//        //проверяем наличие кнопки Выйти и кнопка меню Три полоски
//        onView(withId(buttonMenuId)).perform(click());
//        onView(withId(logoutButtonId)).perform(click());
//    }
